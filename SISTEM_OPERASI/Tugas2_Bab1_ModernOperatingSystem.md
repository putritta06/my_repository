...
<br>
<h1 align="center"> Modern Operation Sistem</h1>
<br>
<br>
<br>
<br>
...
<h3 align="center">HERBERT BOS </h3>
<h3 align="center">Vrije Universiteit </h3>
<h3 align="center">Amsterdam, The Netherlands</h3>
<br>
<br>
<br>
<br>
...
<h6 align="center">Boston Columbus Indianapolis New York San Francisco Upper Saddle River
Amsterdam Cape Town Dubai London Madrid Milan Munich Paris Montréal Toronto
Delhi Mexico City São Paulo Sydney Hong Kong Seoul Singapore Taipei Tokyo</h6>
<br>
...
The lseek call changes the value of the position pointer, so that subsequent calls to read or wr ite can begin anywhere in the file.</p>

<p align="justify">&nbsp &nbsp &nbsLseek has three parameters: the first is the file descriptor for the file, the second is a file position, and the third tells whether the file position is relative to the beginning of the file, the current position, or the end of the file. The value returned
by lseek is the absolute position in the file (in bytes) after changing the pointer.</p>

<p align="justify">&nbsp &nbsp &nbsFor each file, UNIX keeps track of the file mode (regular file, special file, directory, and so on), size, time of last modification, and other information. Programs can ask to see this information via the stat system call. The first parameter
...
<p align="justify">&nbsp &nbsp &nbsIn this section we will look at some system calls that relate more to directories
...
<p align="justify">&nbsp &nbsp &nbsTo see how link works, consider the situation of Fig. 1-21(a). Here are two
...
<p align="center"><img src="gambar/gambar20.png"></p>

<p align="justify"> &nbsp &nbsp &nbsUnderstanding how link works will probably make it clearer what it does.
...
<p align="justify"> &nbsp &nbsp &nbsAs we have mentioned earlier, the mount system call allows two file systems to
...
<p align="justify"> &nbsp &nbsp &nbsBy executing the mount system call, the USB file system can be attached to the
root file system, as shown in Fig. 1-22. A typical statement in C to mount is</p>

 mount("/dev/sdb0", "/mnt", 0);</p>
...
<p align="center>"><img src="gambar/gambar21.png"></p>

<p align="justify">&nbsp &nbsp &nbsAfter the mount call, a file on drive 0 can be accessed by just using its path
...
<br>
<p align="justify">&nbsp &nbsp &nbsA variety of other system calls exist as well. We will look at just four of them here. The chdir call changes the current working directory. After the call</p>

 chdir("/usr/ast/test");</p>

<p align="justify">an open on the file <i>xyz</i> will open <i>/usr/ast/test/xyz</i>. The concept of a working directory eliminates the need for typing (long) absolute path names all the time.</p>

<p align="justify">&nbsp &nbsp &nbsIn UNIX every file has a mode used for protection. The mode includes the
...
<p align="justify">&nbsp &nbsp &nbsThe kill system call is the way users and user processes send signals. If a process is prepared to catch a particular signal, then when it arrives, a signal handler is run. If the process is not prepared to handle a signal, then its arrival kills the process (hence the name of the call).</p>

<p align="justify">&nbsp &nbsp &nbsPOSIX defines a number of procedures for dealing with time. For example,
...
<p align="justify">&nbsp &nbsp &nbsSo far we have focused primarily on UNIX. Now it is time to look briefly at
...
<p align="justify">&nbsp &nbsp &nbsOf course, Windows also has system calls. With UNIX, there is almost a oneto-one relationship between the system calls (e.g., read) and the library procedures
...
<p align="justify">&nbsp &nbsp &nbsWith Windows, the situation is radically different. To start with, the library
...
<p align="justify">&nbsp &nbsp &nbsThe number of Win32 API calls is extremely large, numbering in the thousands. Furthermore, while many of them do invoke system calls, a substantial number are carried out entirely in user space. As a consequence, with Windows it is
...
<p align="justify">&nbsp &nbsp &nbsThe Win32 API has a huge number of calls for managing windows, geometric
...
<p align="justify">&nbsp &nbsp &nbsEven introducing all the Win32 API calls here is out of the question, so we will
...
<p align="justify">&nbsp &nbsp &nbsLet us now briefly go through the list of Fig. 1-23. CreateProcess creates a
...
<p align="justify">&nbsp &nbsp &nbsThe next six calls operate on files and are functionally similar to their UNIX
...
<p align="justify">&nbsp &nbsp &nbsWindows has directories and they are created with CreateDirector y and
...
<p align="justify">&nbsp &nbsp &nbsThe Win32 interface does not have links to files, mounted file systems, security, or signals, so the calls corresponding to the UNIX ones do not exist. Of course,
...
<p align="justify">&nbsp &nbsp &nbsOne last note about Win32 is perhaps worth making. Win32 is not a terribly
...
<p align="justify">&nbsp &nbsp &nbsNow that we have seen what operating systems look like on the outside (i.e.,
...
<p align="justify">&nbsp &nbsp &nbsBy far the most common organization, in the monolithic approach the entire
...
<p align="center"><img src="gambar/gambar22.png"</p>
...
<p align="center"><img src="gambar/gambar23.png"></p>
...
<p align="justify">One interesting server is the <b>reincarnation server</b>, whose job is to check if the
...
<p align="justify">A slight variation of the microkernel idea is to distinguish two classes of processes, the <b>servers</b>, each of which provides some service, and the <b>clients</b>, which use
...
<p align="center"><img src="gambar/gambar24.png"></p>
...
<p align="center"><img src="gambar/gambar25.png"></p>
...
<p align="center"><img src ="gambar/gambar26.png"></p>
...
<p align="justify">Some of these early research projects improved the performance over interpreters like <i>Bochs</i> by translating blocks of code on the fly, storing them in an internal cache, and then reusing them if they were executed again. This improved the
...
<p align="justify">&nbsp &nbsp &nbspWhen the guest operating system is booted, it does the same thing it does on
...
<p align="justify">&nbsp &nbsp &nbspA different approach to handling control instructions is to modify the operating
...
<p align="justify">&nbsp &nbsp &nbspAnother area where virtual machines are used, but in a somewhat different
...
### 1.7.6 Exokernels
<br>
<p align="justify">&nbsp &nbsp &nbspway, is for running Java programs. When Sun Microsystems invented the Java prgramming language, it also invented a virtual machine (i.e., a computer architecture) called the JVM (Java Virtual Machine). The Java compiler produces code
...
<p align="justify">&nbsp &nbsp &nbspAt the bottom layer, running in kernel mode, is a program called the <b>exokernel</b>
...
<p align="justify">&nbsp &nbsp &nbspThe advantage of the exokernel scheme is that it saves a layer of mapping. In
...
<br>
<p align="justify">&nbsp &nbsp &nbspOperating systems are normally large C (or sometimes C++) programs consisting of many pieces written by many programmers. The environment used for
...
<p>This is not a guide to C, but a short summary of some of the key differences
...
<p>One feature C has that Java and Python do not is explicit pointers. A pointer is a variable that points to (i.e., contains the address of) a variable or data structure.
...
<p>Some things that C does not have include built-in strings, threads, packages,
...
<p> An operating system project generally consists of some number of directories,
...
<br>
<p>To build the operating system, each .<i>c</i> is compiled into an <b>object file</b> by the C
...
<p>The first pass of the C compiler is called the <b>C preprocessor</b>. As it reads each
...
<p>Since operating systems are very large (fiv e million lines of code is not
...
<p>Fortunately, computers are very good at precisely this sort of thing. On UNIX
...
<p>Once all the .<i>o</i> files are ready, they are passed to a program called the <b>linker</b> to
...
<p align="center"><img src="gambar/gambar27.png"></p>

<h3>1.8.4 The Model of Run Time</h3>

<p>Once the operating system binary has been linked, the computer can be
...
<p>In all cases, the operating system code is directly executed by the hardware,
...
published by either ACM, the IEEE Computer Society, or USENIX and are available over the Internet to (student) members of these organizations. For more information about these organizations and their digital libraries, see
 ...
 <p>Computer Society     http://www.computer.org
 <P> USENIX               http://www.usenix.org
...
<p>We hav e now completed our introduction and bird’s-eye view of the operating
...
<p>Chapter 2 is about processes and threads. It discusses their properties and how
...
<p>In Chap. 3 we will study address spaces and their adjunct, memory management, in detail. The important topic of virtual memory will be examined, along
with closely related concepts such as paging and segmentation.</p>

<p>sp Then, in Chap. 4, we come to the all-important topic of file systems. To a considerable extent, what the user sees is largely the file system. We will look at both
the file-system interface and the file-system implementation.</p>

<p>Input/Output is covered in Chap. 5. The concepts of device independence and
...
<p>To avoid any confusion, it is worth stating explicitly that in this book, as in
...
<p align="justify">&nbsp &nbsp &nbspIt is also worth pointing out that, in common industry practice, the units for
...
1-KB memory contains 10^24 bytes, not 1000 bytes. Similarly, a 1-MB memory
...
for 10^3, 10^6, and 109 bits/sec, respectively.</p>

## 1.12 SUMMARY

<p align="justify">&nbsp &nbsp &nbsp Operating systems can be viewed from two viewpoints: resource managers and
...
<p align="justify">&nbsp &nbsp &nbsp Operating systems have a long history, starting from the days when they replaced the operator, to modern multiprogramming systems. Highlights include
early batch systems, multiprogramming systems, and personal computer systems.</p>

<p align="justify">&nbsp &nbsp &nbsp Since operating systems interact closely with the hardware, some knowledge
...
<p align="justify">&nbsp &nbsp &nbspThe basic concepts on which all operating systems are built are processes,
memory management, I/O management, the file system, and security. Each of these
will be treated in a subsequent chapter.</p>

<p align="justify">&nbsp &nbsp &nbspThe heart of any operating system is the set of system calls that it can handle.
...
<p align="justify">&nbsp &nbsp &nbspOperating systems can be structured in several ways. The most common ones
